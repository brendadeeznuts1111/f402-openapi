...
## Typed navigation (24 tabs / 7 groups)

Source: `js/lib/navigation-config.js` + `navigation-schemas.js`. Snapshot: `snapshots/navigation-config.snap.json`. PWA: `public/manifest.json` → `navigation.groupTabs`. Registry edge cases: `metadata/navigation-registry.json`.

## Sidebar views (`view-routes.json`)

Legacy `data-view` panels — `overview`, `analytics`, `logs`, `settings`, `endpoints`, `activity`, `data`, `pending`, `alerts`, `customers`, `transactions`. New tabs map via `viewId` on nav items.
...
| `navigation-snapshots.test.js` | `SidebarConfig`, `TAB_PATHS`, `PATH_TO_TAB` snapshot |
| `zod-navigation.test.js` | Nav Zod schemas + `navigation-registry.json` |
| `openapi-navigation.test.js` | `openApiOperationId` + `workerApiPath` vs worker manifest |
| `openapi-schemas.test.js` | TAB_PATHS uniqueness + per-tab API anchors |
| `navigation-performance.test.js` | Full `SidebarConfig` Zod parse (10% threshold) |
...
npm run test:harness          # all harness tests (81+)
...
- `navigation-config.snap.json` — typed sidebar config + path maps

Do not edit by hand. Run `npm run test:harness:update` when shapes change on purpose.

- `performance-baseline.json` — Zod parse timing baselines (15% default; navigation 10%)
