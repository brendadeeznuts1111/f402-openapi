...
When pasting a full browser `Cookie` header, send it as `cookieHeader`; `/refresh-auth` extracts any application session cookie into `sessionCookie` and the Cloudflare cookies into their dedicated fields without echoing values. If diagnostics reports `upstreamAuthShape.ingestionReadiness.status = "blocked"`, refresh the browser request so it includes unexpired bearer authorization plus both Cloudflare cookies. Diagnostics includes `upstreamAuthShape.authorizationExpiry` and `upstreamAuthShape.browserHeaders`, sanitized reports for bearer lifetime and expected browser header names; it never includes token, cookie, or header values.
